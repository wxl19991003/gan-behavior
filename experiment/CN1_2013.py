...
dataframe = pd.read_csv("log/rawdata2013.csv", low_memory=False, index_col=0)
npdata = dataframe.values

data1=pd.read_csv('data/missing_df_full_2013_02_8859.csv',usecols=['CaseID','Activity'])
di=data1.isna()
di.values
...
rdata=pd.read_csv('data/bpi_2013.csv',usecols=['Activity'])
rdata=rdata.values
data1=data1.dropna()
data1=data1.values
...
if __name__ == '__main__':
    ...
    d = np.array(d)
    dtr, dte = model_selection.train_test_split(d, train_size=0.77)
    dtr = list(dtr)
    dte = list(dte)
    ...
    acclist31.append(trainandtest(train_loader1, test_loader1, CNN_Net1, loss_func, 0.001, train_dataset1, 100,device))
    # acclist31.append(trainandtest(train_loader2, test_loader2, CNN_Net2, loss_func, 0.001, train_dataset1, 100,device))
    # acclist31.append(trainandtest(train_loader3, test_loader3, CNN_Net3, loss_func, 0.001, train_dataset1, 100,device))
    print(acclist31)
    end_time=time()
