...
dataframe = pd.read_csv("log/RoadFine_pre.csv", low_memory=False, index_col=0)
    ...
    for epoch in range(epochs):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            if i % 200 == 0:
                ...
                # torch.save(model,'roadfine_CNN_new'+str(lrc)+model.getstr())
        running_loss /= len(train_dataset)
        running_acc /= len(train_dataset)
        print("[%d/%d] Loss: %.5f, Acc: %.5f" % (epoch + 1, epochs, running_loss, 100 * running_acc))
        if epoch==10 and 100 * running_acc<98:
            break
    torch.save(model, 'roadfinemodel/roadfine_1_7' + model.getstr()+'_'+str(ix)+'th')
    model.eval()
    testloss = 0
    testacc = 0
    outlist = []
...
if __name__ == '__main__':
    ...
    datatraintrace1.to_csv('roadfinedata/datatraintrace1.csv')
    datatesttrace2 = pd.DataFrame(datatesttrace)
    datatesttrace2.to_csv('roadfinedata/datatesttrace2.csv')
    label1 = pd.DataFrame(trainlabel1)
    label1.to_csv('roadfinedata/label1.csv')
    label2 = pd.DataFrame(trainlabel2)
    label2.to_csv('roadfinedata/label2.csv')
    ...
    start_time = time()
    # for i in range(30):
    #     acclist31.append(trainandtest(train_loader1, test_loader1, CNN_Net1, loss_func, 0.001, train_dataset1, 50,device,i))
    #     acclist32.append(trainandtest(train_loader2, test_loader2, CNN_Net2, loss_func, 0.001, train_dataset1, 50,device,i))
    #     acclist33.append(trainandtest(train_loader3, test_loader3, CNN_Net3, loss_func, 0.001, train_dataset1, 50,device,i))
    pd1,prelist=loadmodelandpre(test_loader1,'roadfinemodel/roadfine_CNN_new0.001CNN_Net1',device)
    pd2,_=loadmodelandpre(test_loader2,'roadfinemodel/roadfine_CNN_new0.001CNN_Net2',device)
    pd3,_=loadmodelandpre(test_loader3,'roadfinemodel/roadfine_CNN_new0.001CNN_Net3',device)
    print(pd1)
    print(pd2)
    print(pd3)
    print(prelist[:10])
    # print(acclist31, max(acclist31), acclist31.index(max(acclist31)))
    # print(acclist32, max(acclist32), acclist32.index(max(acclist32)))
    # print(acclist33,max(acclist33),acclist33.index(max(acclist33)))
    # print(acclist31)
    end_time=time()
