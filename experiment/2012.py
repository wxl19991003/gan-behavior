...
data1=pd.read_csv('data/missing_df_full_2012_10_9087.csv',usecols=['CaseID','Activity'])
di=data1.isna()
di.values
...
rdata=pd.read_csv('data/bpi_2012.csv',usecols=['Activity'])
rdata=rdata.values
data1=data1.dropna()
data1=data1.values
...
def getsln11(trace, eventlist,datatraceindex):
    sln = []
    length = len(trace)
    tag=0
    l1 = []
