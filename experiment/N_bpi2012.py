...
dataframe = pd.read_csv("log/r_bpi2012.csv", low_memory=False, index_col=0)
...
def getmultigraph(das, eventlist, lc, le, ls, blanklist):
    actgraph = []
    lln = []
    lls = []
    llc = []
...
class CNN_Net1(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(6, 8, kernel_size=3)
        self.conv2 = nn.Conv2d(8, 32, kernel_size=4)
        self.conv2_drop = nn.Dropout2d()
        self.fc1 = nn.Linear(9408, 360)
        self.fc2 = nn.Linear(360, 36)
...
if __name__ == '__main__':
    ...
    datatrain, datatest = model_selection.train_test_split(npdata, train_size=0.6)
    ...
    datatraintrace, datatrainlabel = delevent(data)
    ...
    multitrain1, multitrain2, multitrain3 = getmultigraph(datatraintrace, eventlist, lc, le, ls, blanklist)
    multitest1, multitest2, multitest3 = getmultigraph(datatesttrace, eventlist, lc, le, ls, blanklist)
    # trainlabel1,trainlabel2=[],[]
    # for i in range(len(datatrainlabel)):
    #     l=[]
    #     l.append(datatrainlabel[i])
