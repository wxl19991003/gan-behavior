...
class CNN_Net1(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(8, 32, kernel_size=8)
        self.conv2_drop = nn.Dropout2d()
        self.fc1 = nn.Linear(34944, 1800)
        self.fc2 = nn.Linear(1800, 33)
...
def trainandtest(train_loader,test_loader,cnn,loss_func,lrc,train_dataset,epochs,device,xi):
    model=cnn().to(device)
    opt= torch.optim.SGD(model.parameters(), lr=lrc, momentum=0.99)
    trainloss_count = []
    for epoch in range(epochs):
        ...
        # torch.save(model,'log1model/log1_'+str(xi)+'th_'+str(lrc)+model.getstr())
        running_loss /= len(train_dataset)
        running_acc /= len(train_dataset)
        print("[%d/%d] Loss: %.5f, Acc: %.5f" % (epoch + 1, epochs, running_loss, 100 * running_acc))
        if epoch==15 and running_acc<96:
            break
    torch.save(model, 'log1model/log1_' + str(xi) + 'th_' + str(lrc) + model.getstr())
    model.eval()
    testloss = 0
...
if __name__ == '__main__':
    ...
    datatraintrace1.to_csv('log1/datatraintrace1.csv')
    datatesttrace2 = pd.DataFrame(datatesttrace)
    datatesttrace2.to_csv('log1/datatesttrace2.csv')
    label1 = pd.DataFrame(trainlabel1)
    label1.to_csv('log1/label1.csv')
    label2 = pd.DataFrame(trainlabel2)
    label2.to_csv('log1/label2.csv')
    ...
    acclist31.append(loadmodelandpre(test_loader1,'log1/log1_1th_0.001CNN_Net1',device))
    acclist31.append(loadmodelandpre(test_loader2, 'log1/log1_7th_0.001CNN_Net2', device))
    acclist31.append(loadmodelandpre(test_loader3, 'log1/log1_19th_0.001CNN_Net3', device))
    acclist31.append(loadmodelandpre(test_loader1, 'log1/log1_CNN_new0.001CNN_Net1', device))
    print(acclist31)
    # print(acclist32)
    # print(acclist33)
    end_time=time()
