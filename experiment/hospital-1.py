...
dataframe = pd.read_csv("log/hospital.csv", low_memory=False, index_col=0)
    ...
    for epoch in range(epochs):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            if i % 200 == 0:
                ...
                torch.save(model,'hospital_CNN_new'+str(lrc)+model.getstr())
...
if __name__ == '__main__':
    ...
    datatraintrace1.to_csv('hospital/datatraintrace1.csv')
    datatesttrace2 = pd.DataFrame(datatesttrace)
    datatesttrace2.to_csv('hospital/datatesttrace2.csv')
    label1 = pd.DataFrame(trainlabel1)
    label1.to_csv('hospital/label1.csv')
    label2 = pd.DataFrame(trainlabel2)
    label2.to_csv('hospital/label2.csv')
    ...
    start_time = time()
    # acclist31.append(trainandtest(train_loader1, test_loader1, CNN_Net1, loss_func, 0.003, train_dataset1, 50,device))
    # acclist31.append(trainandtest(train_loader2, test_loader2, CNN_Net2, loss_func, 0.003, train_dataset1, 50,device))
    # acclist31.append(trainandtest(train_loader3, test_loader3, CNN_Net3, loss_func, 0.003, train_dataset1, 50,device))
    # print(acclist31)
    # for i in range(5):
    acclist31.append(loadmodelandpre(test_loader1,'hospital_CNN_new0.001CNN_Net1',device))
    acclist31.append(loadmodelandpre(test_loader2, 'hospital_CNN_new0.001CNN_Net2', device))
    acclist31.append(loadmodelandpre(test_loader3, 'hospital_CNN_new0.003CNN_Net3', device))
    print(acclist31)
    # print(acclist32)
    # print(acclist33)
    end_time=time()
